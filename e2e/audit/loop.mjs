...
// delegated to the Claude Code CLI when present; otherwise the loop runs in
// report-only mode and surfaces an evidence-rich report for a human/agent.
//
// SAFE BY DEFAULT: bare `node loop.mjs` only runs → classifies → reports. The
// self-healing (recursive Claude CLI) and auto-commit are OPT-IN, because each
// edits/commits autonomously. Full autonomous mode is `--heal --commit`.
//
// Guardrails:
...
function runSuite() {
  ...
  // Default headless so visual comparisons match the committed (headless) baselines
  // and CI. --headed is an explicit opt-in for watching functional flows.
...
function heal(finding) {
  ...
  const r = spawnSync(
    "claude",
    ["-p", prompt, "--permission-mode", "acceptEdits", "--allowedTools", "Read,Edit,Grep,Glob"],
...
async function main() {
  ...
  const canHeal = claudeAvailable();
  log(`mode: ${canHeal ? "heal (claude CLI)" : "report-only"} | commit: ${OPTS.commit ? (OPTS.dryRun ? "dry-run" : "on-green") : "off"}`);
  if (OPTS.heal && !canHeal) log("--heal requested but claude CLI not found → report-only");
  ...
  while (iterations < OPTS.maxIterations) {
    ...
    if (!canHeal) {
      log("no healer available — writing report and stopping");
      break;
    }
