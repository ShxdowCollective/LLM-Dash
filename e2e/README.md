...
```bash
HEADLESS=1 npm run e2e # deterministic local full suite
npm run e2e            # headed local run when a display is available
npm run e2e:headed     # watch it run
npm run e2e:ui         # Playwright UI mode
npm run e2e:update     # regenerate visual baselines (headless)
...
npm run audit:fix      # FULL AUTONOMOUS: --heal (OpenCode free model) + --commit on green
```
...
- `--heal` — delegate one fix per failure to **OpenCode** driven with a free
  coding model. The loop bootstraps OpenCode if it's missing (`npm i -g
  opencode-ai`, or bun/curl), then picks a `*-free` coding model. The healer is
  forbidden from running git. With no route available it stays report-only.
