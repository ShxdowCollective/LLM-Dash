...
- Update `TODO.md` and the relevant docs when development work changes
  behavior, workflow, architecture, or user-facing expectations.
...
- Design release, test, and publish flows to work without GitHub Actions. Do not
  add or depend on GitHub Actions unless the user explicitly asks for it. The
  primary verification path should be documented local commands that an agent or
  maintainer can run directly.
...
- **Run the server (dev):** `llm-dash start` after `./install.sh` /
 `.\install.bat`, or `.venv/bin/python -m uvicorn server:app --host 127.0.0.1 --port 8787`
 for a direct dev server.
