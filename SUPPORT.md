# Support

Thanks for using LLM-Dash. Here's where to go depending on what you need.

## Questions & Help

- **How-to questions, ideas, show-and-tell** → open a
  [GitHub Discussion](https://github.com/phxntomkid/LLM-Dash/discussions)
  (if enabled) or a question issue.
- **Check the docs first** — most setup and usage answers live in:
  - [README](README.md) — quick start and overview
  - [docs/USAGE.md](docs/USAGE.md) — install variants, background mode, reset,
    update workflow, full troubleshooting
  - [docs/DEVELOPMENT.md](docs/DEVELOPMENT.md) — local setup and contributor guide
  - [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) — how the system fits together

## Bugs & Feature Requests

- **Found a bug?** Open a
  [bug report](https://github.com/phxntomkid/LLM-Dash/issues/new?template=bug_report.yml).
- **Want a feature?** Open a
  [feature request](https://github.com/phxntomkid/LLM-Dash/issues/new?template=feature_request.yml).

Before filing, search existing [issues](https://github.com/phxntomkid/LLM-Dash/issues)
in case it's already tracked.

## Security

Do **not** report security vulnerabilities through public issues. See
[SECURITY.md](SECURITY.md) for private disclosure.

## Response Expectations

LLM-Dash is maintained on a best-effort basis. There's no SLA — be patient and
provide enough detail (OS, Python version, launch method, logs) to make your
report reproducible.
